...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        self.linear_classifer = nn.Linear(config.hidden_size, config.num_labels)


    def forward(self, input_ids, attention_mask):
        # todo
        bert_encoded_output = self.bert(input_ids, attention_mask)
        pooled_output = bert_encoded_output["pooler_output"]
        pooled_output = self.dropout(pooled_output)
        pooled_output = self.linear_classifer(pooled_output)
        softer = nn.LogSoftmax(dim=1)
        pooled_output = softer(pooled_output)
