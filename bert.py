...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    scores = scores.masked_fill(attention_mask == -10000, -math.inf)
    ...
    softer = nn.Softmax(dim=3)

    scores = softer(scores)
    ...
    scores = scores.view(query.size(0), -1, self.all_head_size)
    return scores
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    attended_output = self.self_attention(hidden_states, attention_mask)

    # add-norm layer
    attended_output = self.add_norm(hidden_states, attended_output, self.attention_dense, self.attention_dropout,
                                 self.attention_layer_norm)

    # feed forward
    feed_output = self.interm_dense(attended_output)
    feed_output = self.interm_af(feed_output)

    # another add-norm layer
    feed_output =  self.add_norm(attended_output, feed_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    return feed_output
    raise NotImplementedError
